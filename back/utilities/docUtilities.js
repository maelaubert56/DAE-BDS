...
const daeFiller = async (data) => {
  ...
  ); // create the path of the template document

  var filledDAEPath = path.join(
    __dirname,
    `../files/forms/filled/DAE_to_review_${data.prenom}_${data.nom}_${data.date}.docx`
  ); // create the path of the filled document
  ...
  for (let i = 0; i < data.courses.length; i++) {
    courses +=
      "- " +
      data.courses[i].matiere +
      " de " +
      data.courses[i].heureDebut +
      " à " +
      data.courses[i].heureFin +
      "\n";
  }
  data.cours = courses;

  // for now : date = '2024-04-04'
  // change the date format to '4 avril 2024'
  ...
  data.fait_le = new Date().toLocaleDateString("fr-FR", {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
  ...
  fs.writeFileSync(filledDAEPath, buf); // write the buffer to the filled document (if the file already exists, it will be overwritten)
...
const docxToPdf = async (docxPath) => {
  try {
    ...
    const pdfPath = path.join(
      __dirname,
      `../files/forms/filled/${fileName}.pdf`
    );
    console.log(pdfPath);
    return pdfPath;
  } catch (error) {

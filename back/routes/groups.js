...
router.get('/', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.execute('SELECT users_groups_name, users_groups_type FROM users_groups');
        const groups = rows;
...
router.post('/', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.execute('SELECT * FROM users_groups WHERE users_groups_name = ?', [req.body.name]);
        if (rows.length > 0) {
            return res.status(409).json({ message: 'Group name already taken' });
        }

        // fetch the user with the email
        const [rows1] = await db.execute('INSERT INTO users_groups (users_groups_name, users_groups_type) VALUES (?,?)', [req.body.name, req.body.type]);
        const group = rows1;
...
router.delete('/:name', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.execute('SELECT * FROM users_groups WHERE users_groups_name = ?', [req.params.name]);
        if (rows.length == 0) {
            return res.status(404).json({ message: 'Group not found' });
        }

        // fetch the user with the email
        const [rows1] = await db.execute('DELETE FROM users_groups WHERE users_groups_name = ?', [req.params.name]);
        const group = rows1;

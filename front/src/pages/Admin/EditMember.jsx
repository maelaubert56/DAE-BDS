...
export default function EditMember() {
    ...
    useEffect(() => {
        if (localStorage.getItem('token')) {
            fetch(`${process.env.REACT_APP_API_URL}/api/users/${window.location.pathname.split('/')[3]}`, {
                method: 'GET',
                headers: {
                    'Authorization': `${localStorage.getItem('token')}`,
                    'Content-Type': 'application/json'
                ...
                setForm(
                    {
                        ...
                        isAdmin: data.user.users_permissions.toString() === '2' || data.user.users_permissions.toString() === '1' ? 1 : 0,
                        ...
                        <div>
                            <div className="d-flex justify-content-between gap-0 gap-sm-3 flex-column flex-sm-row">
                                <Form.Group className="mb-3 w-100" controlId="formBasicEmail">
                                    ...
                                    <Form.Control type="email" placeholder="Enter email" disabled value={form.email} />
                                </Form.Group>
                                <Form.Group className="mb-3 w-100" controlId="formBasicNom">
                                    <Form.Label>Nom</Form.Label>
                                    <Form.Control type="text" placeholder="Nom" onChange={(e) => setForm({ ...form, nom: e.target.value })} value={form.nom} />
                                </Form.Group>
                            </div>
                            <div className="d-flex justify-content-between gap-0 gap-sm-3 flex-column flex-sm-row">
                                <Form.Group className="mb-3 w-100" controlId="formBasicPrenom">
                                    <Form.Label>Prénom</Form.Label>
                                    <Form.Control type="text" placeholder="Prénom" onChange={(e) => setForm({ ...form, prenom: e.target.value })} value={form.prenom} />
                                </Form.Group>
                                <Form.Group className="mb-3 w-100" controlId="formBasicUsername">
                                    <Form.Label>Username</Form.Label>
                                    <Form.Control type="text" placeholder="exemple : Marius Chevailler - Président" onChange={(e) => setForm({ ...form, username: e.target.value })} value={form.username} />
                                </Form.Group>
                            </div>
                            <div className="d-flex justify-content-between gap-0 gap-sm-3 flex-column flex-sm-row">

...
export default function Admin() {
    ...
    const handleAccept = () => {
        ...
        fetch(`${process.env.REACT_APP_API_URL}/api/form/${selectedForm.form_id}`, {
            ...
            .then(data => {
                console.log(data);
                // accept the form
                fetch(`${process.env.REACT_APP_API_URL}/api/form/accept/${selectedForm.form_id}`, {
                    method: 'PUT',
                    headers: {
                        'Authorization': `${localStorage.getItem('token')}`,
                        'Content-Type': 'application/json'
    ...
    const handleSetToWaiting = () => {
        fetch(`${process.env.REACT_APP_API_URL}/api/form/wait/${selectedForm.form_id}`, {
            method: 'PUT',
            headers: {
                'Authorization': `${localStorage.getItem('token')}`,
                'Content-Type': 'application/json'
    ...
    const handleReject = () => {
        ...
        fetch(`${process.env.REACT_APP_API_URL}/api/form/reject/${selectedForm.form_id}`, {
            method: 'PUT',
            headers: {
                'Authorization': `${localStorage.getItem('token')}`,
                'Content-Type': 'application/json'
    ...
    const handleDelete = () => {
        fetch(`${process.env.REACT_APP_API_URL}/api/form/delete/${selectedForm.form_id}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `${localStorage.getItem('token')}`,
                'Content-Type': 'application/json'
        ...
        <>
            <div className="d-flex justify-content-start align-items-center flex-column w-100 h-100 p-3 y-3 gap-3">
                <span className="d-flex flex-column justify-content-between align-items-center w-100 py-1 gap-0">
                    ...
                    <span className='text-center text-secondary w-100'>{forms.length} affichés / {nbForms} au total</span>
                </span>

                <Button variant="primary" onClick={() => setShowFilters(!showFilters)} className={`d-block d-sm-none w-50 ${window.innerWidth < 576 ? 'd-block' : 'd-none'} `}>
                    {showFilters ? "Masquer les filtres" : "Afficher les filtres"}
                </Button>
                <div className={`d-flex flex-row w-100 h-100 gap-3 ${window.innerWidth < 576 ? 'flex-column' : 'flex-row'}`}>
                    <div className={`d-flex justify-content-start align-items-center flex-column border p-3 rounded ${showFilters ? 'mb-3 d-flex' : 'd-none'} ${window.innerWidth > 576 ? 'w-50' : 'w-100'}`} style={{ height: 'fit-content' }}>
                        ...
                        <div className="d-flex justify-content-between gap-3 p-3 w-100 flex-column">
                            <Form.Control type="text" placeholder="Rechercher" onChange={(e) => { setSearchFilter(e.target.value) }} />
                            <Form.Group className={`d-flex mb-3 w-100 ${window.innerWidth < 576 ? 'flex-row gap-2 align-items-center' : 'flex-column'}`}
                                controlId="statut">
                                <Form.Label>Statut</Form.Label>
                                <Form.Select onChange={(e) => { setFiltersStatut(e.target.value) }}>
                                    <option value=''>Tous</option>
                                    <option selected value='to_review'>A review</option>
                                    <option value='accepted'>Accepté</option>
                                    <option value='rejected'>Rejeté</option>
                                </Form.Select>
                            </Form.Group>
                            <Form.Group className={`d-flex mb-3 w-100 ${window.innerWidth < 576 ? 'flex-row gap-2 align-items-center' : 'flex-column'}`} controlId="type">
                                <Form.Label>Type</Form.Label>
                                <Form.Select onChange={(e) => { setFiltersType(e.target.value) }}>
                                    <option value=''>Tous</option>
                                    <option value='DAE'>DAE</option>
                                </Form.Select>
                            </Form.Group>
                            <Form.Group controlId="date" className={`d-flex mb-3 w-100 ${window.innerWidth < 576 ? 'flex-row gap-2 align-items-center' : 'flex-column'}`}>
                                <Form.Label>Date</Form.Label>
                                <div className="w-100 d-flex gap-2 align-items-center">
                                    <Form.Control type="date" onChange={(e) => { setFilterDate(e.target.value) }} />
                                    <CiCircleRemove className="text-secondary" size={'26px'} style={{ cursor: 'pointer' }} onClick={() => { setFilterDate(''); document.querySelector('#date').value = '' }} />

                                </div>
                            </Form.Group>
                            {user?.users_permissions === 1 &&
                                <Form.Group className={`d-flex mb-3 w-100 ${window.innerWidth < 576 ? 'flex-row gap-2 align-items-center' : 'flex-column'}`} controlId="for">
                                    <Form.Label>For</Form.Label>
                                    <Form.Select onChange={(e) => { setFiltersFor(e.target.value) }}>
                                        <option value=''>Tous</option>
                                        {users.map((user, index) => (
                                            <option key={index} value={user.users_email}>{user.users_username}</option>
                                        ))}
                                    </Form.Select>
                                </Form.Group>
                            }
            ...
            >
                <Modal.Header closeButton>
                    ...
                    flex-row justify-content-between w-100 align-items-center">
                        ...
                        <FaSave className="text-primary mx-4" style={{ cursor: 'pointer' }} onClick={() => { handleSaveReview() }} />
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                    {selectedForm && selectedForm?.form_type === 'DAE' ?
                        <div>
                            <Form className={`d-flex justify-content-between flex-column w-100 m-auto ${window.innerWidth < 576 ? 'px-2' : 'px-5'}`}>
                                <div>
                                    <div className="d-flex justify-content-between gap-0 gap-sm-3 flex-column flex-sm-row">
                                        <Form.Group className="mb-3 w-100" controlId="nom">
                                            ...
                                            <Form.Control type="text" placeholder="Entrez votre nom" required value={selectedForm.form_data.nom} onChange={(e) => setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, nom: e.target.value } }))} />
                                        </Form.Group>
                                        <Form.Group className="mb-3 w-100" controlId="prenom">
                                            <Form.Label>Prénom</Form.Label>
                                            <Form.Control type="text" placeholder="Entrez votre prénom" required value={selectedForm.form_data.prenom} onChange={(e) => setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, prenom: e.target.value } }))} />
                                        </Form.Group>
                                    </div>
                                    <div className="d-flex justify-content-between gap-0 gap-sm-3 flex-column flex-sm-row">
                                        <Form.Group className="mb-3 w-100" controlId="classe">
                                            <Form.Label>Classe</Form.Label>
                                            <Form.Control type="text" placeholder="Entrez votre classe" required value={selectedForm.form_data.classe} onChange={(e) => setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, classe: e.target.value } }))} />
                                        </Form.Group>
                                        <Form.Group className="mb-3 w-100" controlId="groupeTD">
                                            <Form.Label>Groupe de TD</Form.Label>
                                            <Form.Control type="text" placeholder="Entrez votre groupe de TD" required value={selectedForm.form_data.groupeTD} onChange={(e) => setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, groupeTD: e.target.value } }))} />
                                        </Form.Group>
                                    </div>
                                    <div className={`d-flex justify-content-between gap-3 ${window.innerWidth < 576 ? 'flex-column' : 'flex-row'}`}>
                                        <Form.Group className="mb-3 w-100" controlId="mail">
                                            <Form.Label>Mail</Form.Label>
                                            <Form.Control type="email" placeholder="Entrez votre mail pour recevoir la DAE" required value={selectedForm.form_data.mail} onChange={(e) => setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, mail: e.target.value } }))} />
                                        </Form.Group>
                                        <Form.Group className="mb-3  w-100" controlId="date">
                                            <Form.Label>Date d'absence</Form.Label>
                                            <Form.Control type="date" required value={selectedForm.form_data.date} onChange={(e) => setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, date: e.target.value } }))} />
                                        </Form.Group>
                                    </div>
                                    <Form.Group className="mb-3 w-100" controlId="motif">
                                        <Form.Label>Motif de l'absence</Form.Label>
                                        <Form.Control as="textarea" rows={3} required value={selectedForm.form_data.motif} onChange={(e) => setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, motif: e.target.value } }))} />
                                    </Form.Group>
                                </div>
                                    ...
                                    {selectedForm?.form_data.courses?.map((course, index) => (
                                        <div key={index} className="mb-3 d-flex justify-content-between flex-column align-items-center w-100 border p-3">
                                            <div className="d-flex justify-content-between w-100">
                                                <p>
                                                    Cours {index + 1}</p>
                                                <CiCircleRemove className="text-secondary"
                                                    style={{ cursor: 'pointer', width: '1.5em', height: '1.5em' }}
                                                    onClick={() => {
                                                        setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, courses: prevState.form_data.courses.filter((course, i) => i !== index) } }))
                                                    }} />
                                            </div>
                                            <Form.Group className="mb-3 w-100" controlId="matiere">
                                                <Form.Label>Code et intitulé du cours concerné</Form.Label>
                                                <Form.Control type="text" placeholder="Entrez le code et l'intitulé du cours" required value={selectedForm.form_data.courses[index].matiere} onChange={(e) => setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, courses: prevState.form_data.courses.map((course, i) => i === index ? { ...course, matiere: e.target.value } : course) } }))} />
                                            </Form.Group>
                                            <div className={`d-flex gap-3 w-100 ${window.innerWidth < 576 ? 'flex-column' : 'flex-row'}`}>
                                                <Form.Group className="mb-3 w-100" controlId="heureDebut">
                                                    <Form.Label>Heure de début</Form.Label>
                                                    <Form.Control type="time" required value={selectedForm.form_data.courses[index].heureDebut} onChange={(e) => setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, courses: prevState.form_data.courses.map((course, i) => i === index ? { ...course, heureDebut: e.target.value } : course) } }))} />
                                                </Form.Group>
                                                <Form.Group className="mb-3 w-100" controlId="heureFin">
                                                    <Form.Label>Heure de fin</Form.Label>
                                                    <Form.Control type="time" required value={selectedForm.form_data.courses[index].heureFin} onChange={(e) => setSelectedForm(prevState => ({ ...prevState, form_data: { ...prevState.form_data, courses: prevState.form_data.courses.map((course, i) => i === index ? { ...course, heureFin: e.target.value } : course) } }))} />
                                                </Form.Group>
                                            </div>
                                        </div>
                                    ))}
                                    <Button variant="secondary" className="w-50 m-auto" onClick={() => {
                                        setSelectedForm(prevState => ({
                                            ...prevState,
                                            form_data: {
                                                ...prevState.form_data,
                                                courses: [...prevState.form_data.courses, { matiere: '', heureDebut: '', heureFin: '' }]
                                            }
                                        }))
                        ...
                        :
                        Object.entries(selectedForm)?.map(([key, value]) => <p>{key} - {value}</p>)
                    }
                </Modal.Body >
                <Modal.Footer className="w-100 d-flex justify-content-end align-items-center flex-row">
                    ...
                    {refuseReasonInput ? <div className="d-flex gap-2 flex-row rejectanddelete">
                        <Form.Control type="text" className='w-100' placeholder="Raison du refus" onChange={(e) => setRefuseReason(e.target.value)} />
                        <Button variant="outline-danger" onClick={handleReject}>
                            Rejeter
                        </Button>
                    </div> :
                        <Button variant="outline-danger" className='rejectanddelete' onClick={() => setRefuseReasonInput(true)}>
                            Rejeter
                        </Button>
                    }
